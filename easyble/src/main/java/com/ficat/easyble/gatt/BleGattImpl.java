...
public class BleGattImpl implements BleGatt {
    ...
    private BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (entry == null) {
                Logger.d("The map entry of address is null");
                return;
            }
    ...
    public synchronized void connect(final BleDevice device, final BleConnectCallback callback) {
        ...
        if (d != null) {
            if (d.connecting) {
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        callback.onStart(false, "The master device is connecting to this device", d);
                    }
                });
                return;
            }
            if (d.connected) {
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        callback.onStart(false, "The master device has already connected to this device", d);
                    }
                });
                return;
            }
        ...
        if (gatt != null) {
            ...
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    callback.onStart(false, "other reason", device);
                }
            });
        }
    }
    ...
    public void notify(final BleDevice device, String serviceUuid, String notifyUuid, final BleNotifyCallback callback) {
        ...
        if (!notify || !indicate) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    callback.onFail(BleCallback.FAIL_OTHER, "this characteristic doesn't support notification or indication", device);
                }
            });
            return;
        }
        if (!enableCharacteristicNotificationOrIndication(gatt, characteristic)) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
    ...
    @Override
    public void writeByBatch(BleDevice device, final String serviceUuid, final String writeUuid, final byte[] writedData, final int lengthPerPackage, final BleWriteByBatchCallback callback) {
    ...
    private List<byte[]> getBatchData(byte[] data, int lengthPerPackage) {
        ...
        int packageNummber = data.length / lengthPerPackage + 1;
    ...
    private boolean checkUuid(String serviceUuid, String charUuid, BluetoothGatt gatt, final BleDevice device, final BleCallback callback) {
        BluetoothGattService service = gatt.getService(UUID.fromString(serviceUuid));
        if (service == null) {
            mHandler.post(new Runnable() {
                @Override
    ...
    private <T> UuidIdentify getUuidIdentifyFromMap(Map<UuidIdentify, T> map, String address, String serviceUuid, String characteristicUuid) {
    ...
    private void removeDeviceCallback(BleDevice device) {
    ...
    private boolean enableCharacteristicNotificationOrIndication(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        if (characteristic == null) {
            return false;
        }
